...
async def get_egresos_financieros(
    ...
    if prestamo_info:
        ...
        reclas_detail = db.execute(text("""
            SELECT subseccion, SUM(monto) AS total
            FROM flujos_reclasificaciones
            WHERE sociedad ILIKE :s
              AND seccion_destino = 'FINANCIAMIENTO'
              AND subseccion IN ('Cuota Capital', 'Liberaciones', 'Intereses Prestamo')
            GROUP BY subseccion
        """), {"s": f"%{sociedad_flujos}%"}).fetchall()

        reclas_por_subsec = {r.subseccion: float(r.total or 0) for r in reclas_detail}
